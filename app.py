...
class FeynmanAI:
    ...
    def score_to_sc_reward(score: int, treasury_balance: int) -> int:
        ...
        Reward capped at AI_REWARD_POOL_PCT of treasury to prevent drain.
        """
        base_reward = FeynmanAI.REWARD_TABLE.get(score, 0)
        if base_reward == 0:
            return 0
        pool_cap = int(treasury_balance * AI_REWARD_POOL_PCT)
        return min(base_reward, pool_cap)
